...
import { deleteSavedReport, readSavedReports, saveReportRecord, type SavedReport } from "../lib/report-storage";
...
function SavedReportsView({ reports, onOpen, onDelete }: { reports: SavedReport[]; onOpen: (report: SavedReport) => void; onDelete: (id: string) => void }) {
...
export default function Home() {
  ...
  const saveCurrentReport = () => { saveReportRecord({ title: report.title, meta: report.meta, sections: report.sections, openaiText: openaiResult?.text, geminiText: geminiResult?.text, templateFilename: template?.filename }); setSavedReports(readSavedReports()); setSaveMessage("보고서를 저장했습니다."); };
  ...
  const removeSavedReport = (id: string) => { deleteSavedReport(id); setSavedReports(readSavedReports()); };
